...
export default function ApologySpace() {
  ...
  const fill = (text) => setMessage(text);
  ...
  const toggleAudio = () => {
    ...
    if (audio.paused) {
      ...
      setIsPlaying(true);
      const interval = setInterval(() => {
        if (audio.ended) { clearInterval(interval); setIsPlaying(false); setCurrentTime(0); }
        else if (audio.paused) { clearInterval(interval); }
  ...
  useEffect(() => {
    if (!code) return;
    const token = localStorage.getItem("token");
    if (!token) router.replace(`/apology/login?code=${code}`);
  }, [code, router]);
  ...
  const uploadAudio = async () => {
    ...
    formData.append("file", audioBlob);
    formData.append("upload_preset", "audio_upload");
    const res = await fetch("https://api.cloudinary.com/v1_1/dgdexbbk7/video/upload", { method: "POST", body: formData });
    const data = await res.json();
    return data.secure_url;
  };

  const sendMessage = async () => {
    if (message.trim() === "") { alert("Please write your apology message 💌"); return; }
    if (receiverName.trim() === "") { alert("Enter receiver name"); return; }
    try {
      const token = localStorage.getItem("token");
      let uploadedAudioUrl = "";
      if (audioBlob) uploadedAudioUrl = await uploadAudio();
      const res = await fetch("/api/messages/save", {
        method: "POST",
      ...
      setToastVisible(true);
      setTimeout(() => { setToastVisible(false); router.push(`/apology/greet?code=${code}`); }, 1500);
      setMessage("");
      setReceiverName("");
      deleteAudio();
    } catch (err) {
  ...
  return (
    <div className="body">
      ...
      {floatingIcons.map((icon, idx) => (
        <div key={idx} className={`apology-float ${icon.className}`} style={{ animationDelay: `${idx * 2}s`, ...icon.style }}>
          {icon.emoji}
        </div>
      ))}
      ...
      <div className="apology-card">
        ...
        <div className="content">
          <div className="hero-section">
            <h1>Let Me Make This Right 💔</h1>
            <p>Sometimes a sincere "I'm sorry" 🤍 can begin the journey of healing 🕊</p>
          </div>
          ...
          <div className={`suggestions-wrapper ${showSuggestions ? "open" : ""}`}>
            <div className="suggestions">
              {suggestions.map((sugg, idx) => (
                <div key={idx} className="suggestion-box" onClick={() => fill(sugg)}>{sugg}</div>
              ))}
            </div>
          </div> */}
          ...
          <div className="field">
            <label>Record Your Voice Message 🎙️</label>
            {!isRecording && !audioPreview ? (
              <button type="button" onClick={startRecording} className="record-btn">🎙️ Start Recording</button>
            ) : isRecording ? (
              <button type="button" onClick={stopRecording} className="stop-btn">⏹️ Stop Recording</button>
            ) : null}

            {audioPreview && (
              <>
                <div className="voice-note-label">🎙️ Listen to your recorded apology</div>
                <div className="voice-player">
                  <button onClick={toggleAudio} className="play-btn">{isPlaying ? "⏸" : "▶"}</button>
                  <div className="wave-bar">
                    <div className="progress" style={{ width: duration > 0 ? `${Math.min((currentTime / duration) * 100, 100)}%` : "0%" }} />
                  </div>
                  <span className="time-text">{formatTime(currentTime)} / {formatTime(duration)}</span>
                  <audio ref={audioRef} src={audioPreview} preload="auto" />
                </div>
                <button onClick={deleteAudio} className="delete-btn">🗑️ Delete Recording</button>
              </>
            )}
          </div>

          <button onClick={sendMessage} className="apology-btn">Send My Apology 💝</button>
        </div>
      </div>

      {toastVisible && <div className="toast">💌 Your apology has been sent with love.</div>}

      <style jsx>{`
        @import url("https://fonts.googleapis.com/css2?family=Dancing+Script:wght@400;700&family=Poppins:wght@300;400;500;600;700&family=Inter:wght@300;400;500&display=swap");

        * { margin: 0; padding: 0; box-sizing: border-box; }
        :global(html), :global(body) { margin: 0; padding: 0; width: 100%; overflow-x: hidden; }
        ...
        .apology-card {
          ...
          border-radius: 24px;
          padding: 20px 18px;
          box-shadow: 0 30px 70px rgba(255, 107, 180, 0.25), inset 0 1px 0 rgba(255, 255, 255, 0.45);
          border: 1px solid rgba(255, 255, 255, 0.22);
          color: white;
          position: relative;
          z-index: 2;
        ...
        .hero-section { text-align: center; margin-bottom: 10px; }
        .hero-section h1 {
  font-family: "Dancing Script", cursive;
  ...
  text-shadow:
    ...
    0 0 18px rgba(255, 255, 255, 0.18);
        ...
        .suggestions-title.toggle:hover { background: rgba(255, 255, 255, 0.28); transform: translateY(-1px); }
        .dropdown-icon { transition: transform 0.3s ease; color: rgba(255, 255, 255, 0.9); }
        ...
        .record-btn:hover { transform: translateY(-2px); box-shadow: 0 12px 35px rgba(255, 107, 180, 0.55); }
  ...
  color: #fff7f7;
  text-shadow:
    0 2px 8px rgba(196, 69, 105, 0.35),
  ...
  animation: fadeInSoft 0.8s ease;
        ...
        .voice-player {
          ...
          border-radius: 26px;
          background: linear-gradient(135deg, #ff6b9d, #c44569);
          box-shadow: 0 8px 25px rgba(196, 82, 207, 0.4);
          transition: all 0.3s ease;
        }
        .voice-player:hover { transform: translateY(-2px); }
        ...
        .play-btn:hover { transform: scale(1.1); background: #fff8dc; }
        ...
        .delete-btn:hover { background: rgba(255, 77, 77, 0.35); transform: translateY(-1px); }
        ...
        .apology-btn {
          ...
          font-family: "Poppins", sans-serif;
          transition: all 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94);
          box-shadow: 0 10px 28px rgba(215, 109, 119, 0.5);
          text-transform: uppercase;
          letter-spacing: 1px;
        }
        .apology-btn:hover { transform: translateY(-3px) scale(1.02); box-shadow: 0 16px 40px rgba(230, 0, 255, 0.5); }
